...
function renderAlertButton(
  ...
  return `
    ...
    >
      <span class="block text-sm font-extrabold text-[var(--panel-text)]">${enabled ? siteText("alertOn") : siteText("alertOff")} · ${label}</span>
  ...
  return `
    <section id="member-alerts-section" class="theme-smooth rounded-[14px] border border-[var(--admin-border)] bg-[var(--panel-bg)] p-4 text-[var(--panel-text)] shadow-[0_4px_14px_rgba(31,36,51,0.06)] md:p-5">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div class="min-w-0">
          <p class="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[var(--header-accent)]">${siteText("memberAlertsEyebrow")}</p>
          <h2 class="mt-1 text-[20px] font-extrabold leading-tight">${siteText("memberAlertsTitle")}</h2>
          <p id="member-alerts-status" class="mt-1 text-sm text-[var(--panel-muted)]">
            ${session ? siteText("signedInAs").replace("{email}", session.email) : siteText("memberAlertsSignedOut")}
          </p>
        </div>
        <div class="grid shrink-0 gap-2 sm:grid-cols-2 md:min-w-[420px]">
          ${renderAlertButton("member-alert-promo", prefs.notifyPromoCodes, siteText("rslPromoCodes"), siteText("rslPromoCodesDescription"))}
          ${renderAlertButton("member-alert-posts", prefs.notifyPosts, siteText("newPosts"), siteText("newPostsDescription"))}
        </div>
      </div>
    </section>`;
}
